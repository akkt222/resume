...
実際に手を動かしてサービス開発を行うことはもちろん、チームビルディングやDDDを利用した既存サービスのモデリング等に取り組んでいます。
...
全文検索の導入や既存インフラのIaC化、非同期実行基盤の整備や監視基盤の構築などに取り組んでいます。
また品質の向上のための打ち手の実行や、企業フェーズや人員状況を意識したデプロイ戦略の立案・実行を推進しています。
...
決済システムの組込みやGraphQLのquery側実装に取り組んでいます。GolangでGraphQL APIを実装するのが初めてだったので、慣れるまでは少し大変でした。
