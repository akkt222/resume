...
実際に手を動かしてサービス開発を行うことはもちろん、チームビルディングやDDDを利用した既存モノリスのモデリング等に取り組んでいます。

</details>

### 株式会社SAMANSA（2024年1月〜現在）
- 社員ではなく業務委託での参加です。
- 短編映画だけを集めたNetflixのようなアプリ「SAMANSA」の運営を行なっているスタートアップ企業です。
...
全文検索の導入や既存インフラのIaC化、非同期実行基盤の整備などに取り組んでいます。
また品質の向上のための打ち手の実行や、企業フェーズやリソースを意識したデプロイ戦略の立案・実行を推進しています。
